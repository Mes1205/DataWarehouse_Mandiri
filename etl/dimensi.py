...
from etl.wilayah_data import ALL_KOTA, MAP_KOTA, MAP_PROV, MAP_REGION
...
def _transform_dimensi():
    ...
    # -------------------------
    # Kota diekstrak dari transaksi mentah (lalu dibenerin penamaannya via
    # MAP_KOTA), digabung dengan ALL_KOTA (master list kota Indonesia dari
    # berbagai pulau) supaya dim_wilayah selalu lengkap sejak awal walau
    # belum ada transaksi untuk kota tersebut. Provinsi & region/pulau
    # diperkaya dari MAP_PROV/MAP_REGION (etl/wilayah_data.py).
    kotas_trx = df_trx['kota'].dropna().str.strip().replace(MAP_KOTA).unique()
    kotas = pd.unique(pd.concat([pd.Series(ALL_KOTA), pd.Series(kotas_trx)], ignore_index=True))
    df_dim_wilayah = pd.DataFrame({'kota': kotas})
    df_dim_wilayah['provinsi'] = df_dim_wilayah['kota'].map(MAP_PROV).fillna('Unknown')
    df_dim_wilayah['region']   = df_dim_wilayah['kota'].map(MAP_REGION).fillna('Unknown')
